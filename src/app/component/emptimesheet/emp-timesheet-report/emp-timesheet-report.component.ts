...
export class EmpTimesheetReportComponent implements OnInit, AfterViewInit {
  ...
  dataSource = new MatTableDataSource<AllEmployeesTimeSheetReport>(
  ...
  fetchMonthlyReport(event) {
    this.message = false;
    this.allEmployeesTimeSheetReport = [];
    this.dataSource = new MatTableDataSource<AllEmployeesTimeSheetReport>(
      this.allEmployeesTimeSheetReport
    );
    this.selectedDate = moment(event.value).format('DD-MM-YYYY');
    this.timesheetService
      .generateContractBasedTimeSheetReport(this.selectedDate)
      .subscribe((res) => {
        ...
        empTimesheet.forEach((e) => {
          this.allEmployeesTimeSheetReport.push(
            new AllEmployeesTimeSheetReport(
              e.empId,
              e.contractPeriod,
              e.contractId,
              e.contractType,
  ...
  monthlyTimeSheet() {
    this.timesheetService
      .downloadMonthlyTimeSheet(this.selectedDate)
      .subscribe((res) => {
        console.log(res);
