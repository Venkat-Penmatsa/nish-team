...
export class ApplyLeavesComponent implements OnInit {
  ...
  submitLeave() {
    ...
    if (this.leaveForm.valid) {
      ...
      const calculateLeaves: number = this.leaveForm.value
        .numberOfDays as number;
      if (calculateLeaves > numberOfDays) {
        this.error = true;
        this.errorDesc =
          "You don't have enough leave balence for the selected Leave Type.";
        validation = false;
        return;
      }
  ...
  fetchLeaveBalence() {
    let user: any = JSON.parse(localStorage.getItem('user') || '{}');
    this.leavesService.fetchEmpLeaves(user.empId).subscribe((data) => {
      this.leaves = data;
    });
  }
}
