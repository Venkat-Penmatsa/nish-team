...
export class LeavescalculatebatchComponent implements OnInit, OnChanges {
  ...
  unFreezeTimeSheet() {
    ...
    this.leavesService
      .unFreezeTimeSheet(selectedDate, this.user.empName)
      .subscribe((res) => {
        console.log(res);
        this.message = true;
  ...
  triggerSOYLeaves() {
    ...
    let selectedDate = moment(this.selectedYear).format('DD-MM-YYYY');
    this.leavesService
      .triggerSOYLeaves(selectedDate, this.user.empName)
      .subscribe((res) => {
        console.log(res);
        this.message = true;
        this.messageDesc = res.responseStatus + ' ' + res.errorDescription;
  ...
  triggerRTTLeaves() {
    ...
    this.leavesService
      .triggerRTTLeaves(selectedDate, this.user.empName)
      .subscribe((res) => {
        console.log(res);
        this.message = true;
        this.messageDesc = res.responseStatus + ' ' + res.errorDescription;
  ...
  freezeTimeSheetJob() {
    ...
    this.user = JSON.parse(localStorage.getItem('userDetails') || '{}') as User;
    this.leavesService
      .freezeTimeSheetJob(this.freezeTimeSheetDate, this.user.empName)
      .subscribe((res) => {
        console.log(res);
        this.message = true;
        this.messageDesc = res.responseStatus + ' ' + res.errorDescription;
  ...
  freezeNEPtuneTSJob() {
    ...
    this.user = JSON.parse(localStorage.getItem('userDetails') || '{}') as User;
    this.leavesService
      .freezeNEPtuneTSJob(this.freezeTimeSheetDate, this.user.empName)
      .subscribe((res) => {
        console.log(res);
        this.message = true;
        this.messageDesc = res.responseStatus + ' ' + res.errorDescription;
  ...
  fetchNEPtuneTSReport(event) {
    let selectedDate = moment(event.value).format('DD-MM-YYYY');
    this.freezeTimeSheetDate = selectedDate;
    const status = this.batchService(selectedDate, 'FREEZE_NEPTUNE_TIMESHEET');
  }

  batchService(selectedDate: any, batchName: string) {
    this.error = false;
    this.leavesService
      .fetchYearBatchJobStatus(batchName, selectedDate)
      .subscribe((res) => {
        console.log(res);
        if (res.status == 'NOT_EXECUTED' && batchName == 'YEAR_START_LEAVES') {
          ...
          res.status == 'NOT_EXECUTED' &&
          batchName == 'FREEZE_TIMESHEET'
        ) {
          let button = <HTMLButtonElement>(
            document.getElementById('timeSheetFreezeButton')
          );
          button.disabled = false;
        } else if (
          res.status == 'NOT_EXECUTED' &&
          batchName == 'FREEZE_NEPTUNE_TIMESHEET'
        ) {
          let button = <HTMLButtonElement>(
            document.getElementById('tsNEPtuneFreezeButton')
          );
          button.disabled = false;
        } else {
          this.error = true;
          this.errorDesc =
            res.batchJobName +
            ' Batch Job Already Executed for the Period ' +
            res.batchMonth +
            '-' +
            res.batchYear +
            ' On  ' +
