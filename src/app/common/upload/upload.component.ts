...
import {
  ...
  CategoryDocs,
  EmployeeDocs,
} from 'src/app/constants/documentType';
...
export class UploadComponent implements OnInit, OnChanges {
  ...
  fetchDocuments(category: string): any {
    ...
    this.uploadService
      .fetchAllDocuments(category.replace('/', '=='))
      .subscribe((res) => {
        console.log(res);
        res.forEach((e) => {
          this.filesList.push(
  ...
  uploadFiles(): any {
    ...
    if (this.file.length > 1) {
      ...
      this.uploadService.uploadMultiple(formData).subscribe((res) => {
  ...
  onDelete(id: string, path: any) {
    ...
    this.uploadService
      .deleteDocument(id, path.replaceAll('/', '@'))
      .subscribe((res) => {
        console.log(res);
        this.messageService.openSnackBar(
  ...
  onDownload(id: string, path: any) {
    ...
    this.uploadService
      .downloadDocuments(path.replaceAll('/', '@'))
      .subscribe((res) => {
        console.log(res);
        window.open(res.url);
