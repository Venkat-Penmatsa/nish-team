...
export class NewcontractService {
  ...
  createContract(body: any): Observable<any> {
    ...
    return this.http.post(`${this.baseUrl}/contract/newContract`, body, {
      headers,
    });
  }
  ...
  fetchContractInfo(contractId: string) {
    return this.http
      .get(`${this.baseUrl}/contract/fetchContractInfo/` + contractId)
      .pipe(catchError(this.errorHandler));
  }

  getContractReport(): Observable<any> {
    const headers = { 'Content-type': 'application/json' };
    return this.http.get(`${this.baseUrl}/contract/listContractDays`, {
      headers,
    });
  }
